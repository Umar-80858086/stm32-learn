...
#define TICK_HZ 1U //for 1 sec and for 1miliSec its 1000U
...
void idle_task(void)
{
	...
	while(1){
		const char *p = msg;
		        while (*p) {
		            Uart_Write((int)*p++); // typecast each char to int
		        }
	}
}
